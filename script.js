...
(() => {
  ...
  const LOG_SOURCE = Object.freeze({ MANUAL: 'manual', SYSTEM: 'system' });
  ...
  async function handleClick(event) {
    ...
    switch (action) {
      case 'logout':
        await withButtonLoading(target, async () => State.sb.auth.signOut());
        break;
      case 'reload':
        await withButtonLoading(target, async () => refreshCurrentSession(true));
        break;
      case 'retry-boot':
        State.bootError = '';
        initSession();
      ...
      case 'password-toggle':
        ...
        break;
      case 'copy':
        await withButtonLoading(target, async () => copyText(target.dataset.copy || ''));
        break;
      case 'demo-delete':
        await withButtonLoading(target, async () => softDeleteDemo(target.dataset.id));
        break;
      case 'email-preview':
        openEmailPreview(target.dataset.id, target.dataset.type || 'first_demo_email');
        break;
      case 'email-send':
        await withButtonLoading(target, async () => sendPreviewEmail(target.dataset.id, target.dataset.type || 'first_demo_email'));
        break;
      case 'log-edit':
        await withButtonLoading(target, async () => editLog(target.dataset.id));
        break;
      case 'log-delete':
        await withButtonLoading(target, async () => deleteLog(target.dataset.id));
        break;
      case 'report-export':
        await withButtonLoading(target, async () => exportDemoRows());
        break;
      case 'company-report-export':
        await withButtonLoading(target, async () => exportCompanyReportRows());
        break;
      case 'export-current':
      case 'export-all':
        await withButtonLoading(target, async () => exportDemoRows());
      ...
      case 'admin-responsible-toggle':
        await withButtonLoading(target, async () => toggleResponsiblePerson(target.dataset.id, target.dataset.active === 'true'));
        break;
      case 'admin-module-toggle':
        await withButtonLoading(target, async () => toggleModule(target.dataset.id, target.dataset.active === 'true'));
        break;
      case 'admin-module-delete':
        await withButtonLoading(target, async () => deleteModule(target.dataset.id));
        break;
      case 'admin-template-reset':
        await withButtonLoading(target, async () => resetTemplate(target.dataset.key));
        break;
      case 'run-reminder-check':
        await withButtonLoading(target, async () => queueReminderEmails());
        break;
      case 'draft-clear':
  ...
  function renderReportPage() {
    ...
    return `
      ...
      <section class="card report-summary-section">
        <div class="section-title report-section-title">
          <div>
            <h2>สรุปรายบริษัท</h2>
            <p class="muted small-text">แสดงบริษัททั้งหมดโดยเรียงจากวันที่บันทึกล่าสุดก่อน</p>
          </div>
          <div class="field report-search-field">
            <label>ค้นหา</label>
            <input class="input" data-report-search value="${escapeAttr(State.reportSearch)}" placeholder="ค้นหาบริษัท ผู้ติดต่อ โมดูล ผู้รับผิดชอบ หรือบันทึกล่าสุด">
          </div>
        </div>

        ${pageInfo.rows.length ? `
          <div class="report-company-grid">
            ${pageInfo.rows.map((item) => renderCompanyReportCard(item)).join('')}
          </div>
        ` : '<div class="empty soft-empty">ไม่พบข้อมูลบริษัท</div>'}

        ${renderPagination('report', 'company', pageInfo.page, pageInfo.totalPages, rows.length)}
      </section>
    `;
  }

  function renderCompanyReportCard(item) {
    const row = item.row;
    const latestLog = item.latestLog;
    const latestDate = latestLog?.created_at ? formatDateTime(latestLog.created_at) : '-';
    ...
    return `
      <article class="report-company-card">
        <div class="report-company-main">
          ...
          <span>${escapeHTML(row.company.contact_name || '-')}</span>
          <small>${escapeHTML((row.company.contact_emails || []).join(', ') || '-')}</small>
        </div>
        <div class="report-company-demo">
          ${statusBadge(row.effectiveStatus)}
          <span>${formatDate(row.round.start_date)} - ${formatDate(row.round.end_date)}</span>
          <small>${escapeHTML(row.modules.map((module) => module.name).join(', ') || '-')}</small>
        </div>
        <div class="report-latest-note">
          <span class="muted small-text">บันทึกล่าสุด · ${escapeHTML(latestDate)}</span>
          <p>${escapeHTML(latestLog?.message || 'ยังไม่มีบันทึกความคืบหน้า')}</p>
        </div>
        <div class="report-company-actions">
          <a class="btn small ghost" href="#demos/${row.round.id}">ดูรายละเอียด</a>
        </div>
      </article>
    `;
  }
  ...
  function getCompanyReportRows() {
    ...
    return getDemoRows()
      .map((row) => {
        const manualLogs = getManualActivityLogsForCompany(row.company.id);
        const latestLog = manualLogs[0] || null;
        const latestAt = latestLog?.created_at || row.round.updated_at || row.round.created_at || '';
        return { row, latestLog, latestAt };
      })
      .filter((item) => {
  ...
  function exportCompanyReportRows() {
    ...
    const data = rows.map((item) => {
      ...
      return {
        ...
        'สถานะล่าสุด': row.effectiveStatus,
        ...
        'บันทึกล่าสุด': item.latestLog?.message || '',
        'วันที่บันทึกล่าสุด': item.latestLog?.created_at ? formatDateTime(item.latestLog.created_at) : ''
      };
    });
  ...
  function canSendFirstDemoEmail(row, type = 'first_demo_email') {
    if (type !== 'first_demo_email') return true;
    if (userIsAdmin()) return true;
    return !row?.round?.first_email_sent_at;
  }

  function renderEmailActionButton(row, buttonClass = 'btn small ghost') {
    if (canSendFirstDemoEmail(row, 'first_demo_email')) {
      return `<button class="${escapeAttr(buttonClass)}" data-action="email-preview" data-id="${row.round.id}">อีเมล</button>`;
    }
    return '<span class="badge sent">ส่งอีเมลแล้ว</span>';
  }
  ...
  function renderDemoDetail(roundId) {
    ...
    const companyLogs = getManualActivityLogsForCompany(row.company.id);
    ...
    return `
      ${renderTopbar(row.company.company_name, '', `
        ...
        ${renderEmailActionButton(row, 'btn primary')}
  ...
  async function insertActivityLog(payload) {
    const { error } = await State.sb.from('activity_logs').insert({
      ...
      source: payload.source || LOG_SOURCE.MANUAL,
      ...payload,
      created_by: State.profile.id
    });
    if (error) throw error;
  ...
  function openEmailPreview(roundId, type = 'first_demo_email') {
    ...
    const canSend = canSendFirstDemoEmail(row, type);
    ...
    showModal(`
      ...
      <main class="grid">
        ${!canSend ? '<div class="config-warning">รายการนี้ส่งอีเมลแล้ว ผู้ใช้ทั่วไปส่งซ้ำไม่ได้ หากต้องการส่งซ้ำต้องใช้สิทธิ์ผู้ดูแลระบบ</div>' : ''}
      ...
      <footer>
        ...
        ${canSend ? `<button class="btn primary" data-action="email-send" data-id="${roundId}" data-type="${escapeAttr(type)}">ส่งอีเมล</button>` : '<span class="badge sent">ส่งอีเมลแล้ว</span>'}
      </footer>
    `);
  }
  ...
  async function sendPreviewEmail(roundId, type = 'first_demo_email') {
    ...
    if (!canSendFirstDemoEmail(row, type)) {
      closeModal();
      toast('รายการนี้ส่งอีเมลแล้ว หากต้องการส่งซ้ำให้ใช้สิทธิ์ผู้ดูแลระบบ', 'warning');
      render();
      return;
    }
  ...
  function isManualLog(log) {
    return log?.source === LOG_SOURCE.MANUAL;
  }

  function getManualActivityLogsForCompany(companyId) {
    return State.activityLogs
      .filter((log) => log.company_id === companyId && isManualLog(log))
      .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
  }

  function getManualActivityLogsForRound(roundId) {
    return State.activityLogs
      .filter((log) => log.demo_round_id === roundId && isManualLog(log))
      .sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
  }
  ...
  function getAllDemoRows() {
    return State.rounds
      .map((round) => {
        ...
        const logs = getManualActivityLogsForCompany(company.id);
...
ทีม Customer Support`
  ...
  function canModifyLog(log, latestId = null) {
    const companyLogs = getManualActivityLogsForCompany(log.company_id);
  ...
  function formatDateTime(value) {
    if (!value) return '-';
    const text = String(value);
    if (/^\d{4}-\d{2}-\d{2}$/.test(text.slice(0, 10)) && !/[T\s]\d{2}:\d{2}/.test(text)) {
