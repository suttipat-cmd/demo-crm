-- Creates idempotent reminder work items. The function itself is not scheduled;
-- calling it is the only operation that can insert email_logs rows.
...
begin
  ...
  with due_rounds as (
    ...
    left join lateral (
      select da.login_email, da.password, da.note, row_number() over (order by da.created_at, da.id) as ordinality
      from public.demo_accounts da
      where da.demo_round_id = dr.id
    ) a on true
    ...
    where template_key = 'expiry_reminder_email' and is_active = true
    limit 1
  ), payload as (
    select
      d.round_id,
      d.contact_emails,
      array_remove(array[d.responsible_email], null) as cc_emails,
      replace(replace(replace(replace(replace(replace(replace(replace(replace(
        t.subject,
        '{{company_name}}', d.company_name),
        '{{contact_name}}', d.contact_name),
        '{{modules}}', array_to_string(d.module_names, ', ')),
        '{{start_date}}', to_char(d.start_date, 'DD/MM/YYYY')),
        '{{end_date}}', to_char(d.end_date, 'DD/MM/YYYY')),
        '{{remaining_days}}', '3'),
        '{{responsible_name}}', coalesce(d.responsible_name, '')),
        '{{responsible_email}}', coalesce(d.responsible_email, '')),
      ...
      replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(
        ...
        '{{end_date}}', to_char(d.end_date, 'DD/MM/YYYY')),
        '{{remaining_days}}', '3'),
        '{{demo_accounts}}', d.demo_accounts),
        '{{responsible_name}}', coalesce(d.responsible_name, '')),
        '{{responsible_email}}', coalesce(d.responsible_email, '')),
        '{{note}}', d.latest_note) as body
    from due_rounds d cross join template t
  ), inserted as (
    insert into public.email_logs (
      demo_round_id, email_type, to_emails, cc_emails, subject, body,
      sent_status, sent_by, idempotency_key
    )
    select
      p.round_id, 'expiry_reminder_email', p.contact_emails, p.cc_emails, p.subject, p.body,
      'queued', null, 'demo-crm:' || p.round_id::text || ':expiry_reminder_email'
    from payload p
